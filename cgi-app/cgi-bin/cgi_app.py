...
stv = sentence_to_vect(ts)

pred = model.predict([[category_dict['1'],  category_dict['2'], category_dict['10'], category_dict['15'], category_dict['17'], category_dict['19'], category_dict['20'], category_dict['22'], category_dict['23'], category_dict['24'], category_dict['25'], category_dict['26'], category_dict['27'], category_dict['28'], category_dict['29'], int(subscribe_cnt), float(stv)]])
view, likes = int(round(pred[0][0])), int(round(pred[0][1]))

data = pd.read_csv('./100_view_title.csv')
data = data.drop('Unnamed: 0', axis=1)
data = data.sample(n=250)
corpus = []
print(data.columns)
cr = pd.Series([title], index=data.columns)
data = data.append(cr, ignore_index=True)
split_corpus = []
corpus = data['100_view_title']
for title in corpus:
    split_corpus.append(pt_title_split(title))

tfidf_array = pt_sentence_to_vect(split_corpus)
cs = cosine_similarity(tfidf_array,tfidf_array)  # cos類似度計算
cs = cs[-1]
cs = list(map(lambda x: x * 100, cs))
data['percentage'] = cs
data = data.drop(250, axis=0)
data = data.sort_values('percentage', ascending=False)
genrate_title = data.iloc[0, 0]
genrate_per = int(round(data.iloc[0, 1]))
...
print(html_body % (view, likes, genrate_per, genrate_title, propose_video_from_thumbnail[0][1]))